...
class ParentWindow(Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        New_phonebook_func.center_window(self,500,400)
        self.master.title("Student Tracking")
        self.master.configure(bg="#F0F0F0")
        # This protocol method is a tkinter built-in method to catch if 
        # the user clicks the upper corner, "X" on Windows OS.
        self.master.protocol("WM_DELETE_WINDOW", lambda: New_phonebook_func.ask_quit(self))
        arg = self.master

        # load in the GUI widgets from a separate module, 
        # keeping your code comparmentalized and clutter free
        New_phonebook_gui.load_gui(self)
        ...
        filemenu.add_command(label="Exit", underline=1,accelerator="Ctrl+Q",command=lambda: New_phonebook_func.ask_quit(self))
